...
class BackupTestCase(unittest.TestCase):
    ...
    def test__copy_item(self):
        ...
        backup_utility._copy_item(
            self.subFolder1_path, self.destination_dir, verbose=True
        )
    ...
    def test_extract_archive_with_passphrase(self):
        ...
        backup_utility = BackupUtility(
            destination=self.destination_dir, archive=True, passphrase="abc"
        )
        backup_utility.process_list(backup_list, verbose=True)
        ...
        self.assertTrue(
            len(archive_files) > 0,
            "No .7z archive file found in the destination directory",
        )
        ...
        backup_utility.extract_archive_with_passphrase(
            os.path.join(self.destination_dir, archive_file_name),
            self.destination_dir,
            verbose=True,
        )

        # Create path of opened archive
        raw_extracted_archive_path = os.path.join(
            self.destination_dir, archive_file_name
        )
        extracted_archive_path = raw_extracted_archive_path.replace(
            "archive_", ""
        ).replace(".7z", "")
